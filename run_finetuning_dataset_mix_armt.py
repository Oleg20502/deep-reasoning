...
if __name__ == '__main__':
    ...
    # Prepare datasets
    logger.info(f'preparing dataset for {args.task_names}')
    task_names = args.task_names.split(':')
    print(task_names)
    with accelerator.main_process_first():
        tokenized_datasets = [load_and_preprocess_task(task_name, tokenizer, args.sample_size,
    ...
    if not(args.no_packing):
        ...
        with accelerator.main_process_first():
            train_datasets = [ds["train"].map(lambda x: prepare_texts(x, args.truncate_before),
                                                            batched=True, desc=f"Preparing train") for ds in tokenized_datasets]
            if args.truncate_only_train:
                args.truncate_before = 0
            valid_datasets = [ds["validation"].map(lambda x: prepare_texts(x, args.truncate_before),
                                                                batched=True, desc=f"Preparing valid")
                                                                if 'validation' in ds else
                            ds["test"].map(lambda x: prepare_texts(x, args.truncate_before),
                                                                batched=True, desc=f"Preparing valid") for ds in tokenized_datasets]
            test_datasets = [ds["test"].map(lambda x: prepare_texts(x, args.truncate_before),
                                                          batched=True, desc=f"Preparing test") for ds in tokenized_datasets]
            
            
            if args.task_ratios is not None:
                ratios = [float(r) for r in args.task_ratios.split(':')]
            else:
                ratios = args.task_ratios
            train_datasets = combine_datasets(train_datasets, ratios=ratios, target_size=10_000)
            valid_datasets = combine_datasets(valid_datasets, ratios=ratios, target_size=1000)
            test_datasets = combine_datasets(test_datasets, ratios=ratios, target_size=1000)
            train_datasets.save_to_disk('tmp')
