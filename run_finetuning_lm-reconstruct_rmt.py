...
if __name__ == '__main__':
    ...
    with accelerator.main_process_first():
        if args.tokenized_dataset is not None:
            ...
            # dataset = dataset.map(lambda x: tokenizer.encode(x, add_special_tokens=False))
    ...
    def collate_fn(batch):
        input_ids = labels = [torch.cat([torch.tensor(b['input_ids'])] * 2, dim=0) for b in batch]
        attention_mask = [torch.ones_like(b, dtype=int) for b in input_ids]

        labels_mask = [torch.ones_like(b, dtype=int) for b in input_ids]
        for i, lm in enumerate(labels_mask):
            labels_mask[i][:lm.shape[0] // 2] = 0
